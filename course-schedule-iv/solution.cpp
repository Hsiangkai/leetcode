...
public:
  vector<bool> checkIfPrerequisite(int numCourses, vector<vector<int>>& prerequisites,
    ...
    vector<set<int>> pre_courses(numCourses, set<int>());

    while (work_queue.size()) {
      int node = work_queue.front();
      work_queue.pop();
      for (auto adj : graph[node]) {
	pre_courses[adj].insert(node);
	for (auto node_parent : pre_courses[node]) {
	  pre_courses[adj].insert(node_parent);
	}
	in_degrees[adj]--;
	if (in_degrees[adj] == 0) {
	  work_queue.push(adj);
    ...
    for (auto query : queries) {
      int from = query[0];
      int to = query[1];
      set<int> &prereq = pre_courses[to];
      if (prereq.find(from) != prereq.end())
	ret.push_back(true);
      else
	ret.push_back(false);
    }
...
void printOut(vector<bool> &data) {
  for (auto elem : data) {
    cout << elem << " ";
  }
  cout << endl;
